...
WorkerState MeasurementManagerImpl::getWorkerState () const {
...
void MeasurementManagerImpl::StateMachine () {
  ...
  case MeasurementState::enumerate_sensors: {
    ...
    for (auto sensor_bus : _sensor_ring->getInterfaces ()) {
      if (sensor_bus->getSensorCount () == sensor_bus->getEnumerationCount ()) {
        logger::Logger::getInstance ()->log (logger::LogVerbosity::Info, "Counted " + std::to_string (sensor_bus->getEnumerationCount ()) + " of " + std::to_string (sensor_bus->getSensorCount ()) + " sensors on interface " + sensor_bus->getInterface ()->getInterfaceName ());
      } else {
        logger::Logger::getInstance ()->log (logger::LogVerbosity::Info, "Counted " + std::to_string (sensor_bus->getEnumerationCount ()) + " of " + std::to_string (sensor_bus->getSensorCount ()) + " sensors on interface " + sensor_bus->getInterface ()->getInterfaceName ());
  ...
  case MeasurementState::wait_for_data: {
    ...
    if (success) {
      ...
      logger::Logger::getInstance ()->log (
          logger::LogVerbosity::Error, "Timeout occured while waiting for completion of "
                                       "measurements.");
      _measurement_state = MeasurementState::error_handler;
    }
    break;
