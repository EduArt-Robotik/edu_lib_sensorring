	...
	switch (_measurement_state){
		...
		case MeasurementState::enumerate_sensors:
			...
			for(auto sensor_bus : _sensor_ring->getInterfaces()){
				if(sensor_bus->getSensorCount() == sensor_bus->getEnumerationCount()){   
					logger::Logger::getInstance()->log(logger::LogVerbosity::Info, "Counted " + std::to_string(sensor_bus->getEnumerationCount()) + " of " + std::to_string(sensor_bus->getSensorCount()) + " sensors on interface " + sensor_bus->getInterfaceName());
				}else{
					logger::Logger::getInstance()->log(logger::LogVerbosity::Info, "Counted " + std::to_string(sensor_bus->getEnumerationCount()) + " of " + std::to_string(sensor_bus->getSensorCount()) + " sensors on interface " + sensor_bus->getInterfaceName());
		...
		case MeasurementState::wait_for_data:
			...
			if(success){
				...
				logger::Logger::getInstance()->log(logger::LogVerbosity::Error, "Timeout occured while waiting for completion of measurements.");
				_measurement_state = MeasurementState::error_handler;
			}
			break;
		...
		case MeasurementState::fetch_tof_data:
			...
			if(_tof_enabled){
				...
				if(success){
					...
					if (error != 0) logger::Logger::getInstance()->log(logger::LogVerbosity::Warning, "Error occured while parsing tof measurements from " + std::to_string(error) + " sensor(s)");
				}
			}
		...
		case MeasurementState::fetch_thermal_data:
			...
			if(_thermal_enabled and _thermal_measurement_flag){
				...
				if(success){
					...
					if(error != 0) logger::Logger::getInstance()->log(logger::LogVerbosity::Warning, "Error occured while parsing thermal measurements from " + std::to_string(error) + " sensor(s)");
				} 
				_thermal_measurement_flag = false;
			}
