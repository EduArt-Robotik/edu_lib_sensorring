...
void MeasurementManager::registerObserver(MeasurementObserver* observer){
	if(observer){
		...
		if (std::find(_observer_vec.begin(), _observer_vec.end(), observer) == _observer_vec.end()) {
			...
			logger::Logger::getInstance()->log(LogVerbosity::Debug, "Registered new observer");
		}else{
			logger::Logger::getInstance()->log(LogVerbosity::Warning, "Observer already registered");
		}
	}
}
		...
		case MeasurementState::fetch_tof_data:
			...
			if(_tof_enabled){
				...
				if(success){
					...
					if (error != 0) logger::Logger::getInstance()->log(LogVerbosity::Warning, "Error occured while parsing tof measurements from " + std::to_string(error) + " sensor(s)");
				}
			}
		...
		case MeasurementState::fetch_thermal_data:
			...
			if(_thermal_enabled and _thermal_measurement_flag){
				...
				if(success){
					...
					if(error != 0) logger::Logger::getInstance()->log(LogVerbosity::Warning, "Error occured while parsing thermal measurements from " + std::to_string(error) + " sensor(s)");
				} 
				_thermal_measurement_flag = false;
			}
