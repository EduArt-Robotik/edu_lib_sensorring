...
TEST_CASE("DeviceID getters", "[DeviceID]") {
  SECTION("getDeviceType") {
    DeviceID id{DeviceType::HTPA32, "thermal", 2u};
    REQUIRE(id.getDeviceType() == DeviceType::HTPA32);
  }

  SECTION("getDeviceName") {
    DeviceID id{DeviceType::VL53L8CX, "tof_rear", 0u};
    REQUIRE(id.getDeviceName() == "tof_rear");
  }

  SECTION("getDeviceIndex") {
    DeviceID id{DeviceType::WS2812b, "strip", 3u};
    REQUIRE(id.getDeviceIndex() == 3u);
  }
  ...
  SECTION("getters reflect direct member changes") {
    ...
    id.index = 5u;
    REQUIRE(id.getDeviceType() == DeviceType::HTPA32);
    REQUIRE(id.getDeviceName() == "b");
    REQUIRE(id.getDeviceIndex() == 5u);
  }
}
...
TEST_CASE("DeviceID edge cases", "[DeviceID]") {
  ...
  SECTION("long name") {
    ...
    REQUIRE(id.getDeviceName() == long_name);
    REQUIRE(id.isValid());
  }
}
